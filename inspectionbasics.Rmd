...
library(stringr)
inspections %>%
  transmute(InspectionID, tempInspectionType= word(str_to_lower(InspectionType),sep = " |/|-")) %>%
  mutate(total=n()) %>%
  group_by(tempInspectionType) %>%
  mutate(InspectionID, RefinedInspectionType=ifelse(n()>250,tempInspectionType,"Other")) %>%
  ungroup %>%
  select(-tempInspectionType, -total) ->
  inspectiontypes

inspections %>%
  left_join(inspectiontypes, by="InspectionID") ->
  inspections
```
